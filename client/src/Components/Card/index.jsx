...
export default function Card(props) {
  ...
  const [id, setId] = useState( props.id ); //should this be set to null or to id? if an id is present we would want this specific question to be retrieved. Or a random.
  const [answer, setAnswer] = useState(null);

  // make fetch request
  useEffect(() => {
    const route = (id) ? `/question/${id}` : '/random';
